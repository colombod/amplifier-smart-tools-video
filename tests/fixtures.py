"""Test clips, generated by the `ensure_*` helpers below.
...
In practice they ARE tracked in git as well: `alpha`, `bravo`, `charlie`,
`corner`, `keyable` and `silent` all have committed .mp4 files. Generation is
idempotent, so a present file is reused and a missing one is rebuilt; the two
arrangements coexist rather than conflict. This paragraph exists because the
docstring previously claimed they were not committed, which the index
contradicted.
