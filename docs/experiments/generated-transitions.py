...
CASES = [
    ...
    ("fade down to black, then up into the second clip",
     "if(lt(P,0.5), A*(1-2*P), B*(2*P-1))"),
    ("the second clip opens in a circle growing from the centre",
     "if(lt(hypot(X-W/2,Y-H/2), P*hypot(W/2,H/2)), B, A)"),
    ("a diagonal wipe from the top-left corner",
     "if(lt((X/W+Y/H)/2, P), B, A)"),
...
for i, (description, expr) in enumerate(CASES):
    ...
    cmd = ["ffmpeg","-y","-v","error","-i",A,"-i",B,"-filter_complex",
    ...
    if proc.returncode != 0:
        results.append((description, expr, "WILL-NOT-PARSE",
